...
def detect_face(imgs, minsize, pnet, rnet, onet, threshold, factor, device):
    ...
    model_dtype = next(pnet.parameters()).dtype # torch.float32
...
def extract_face(img, box, image_size=160, margin=0, save_path=None, device='cpu'):
    ...
    face = F.to_tensor(np.float32(face)).to(device)
