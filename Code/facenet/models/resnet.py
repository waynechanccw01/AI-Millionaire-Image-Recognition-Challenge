...
class Bottleneck(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.squeeze is None:
            ...
            out = torch.addcmul(residual, out, self.squeeze(out), value=1.0)
...
class ResNet(nn.Module):
    ...
    def load_weights(self, name, device):
        """
        Load pretrained state_dict into model.
        """
        path = f'../data/{name}.pt'
        state_dict_path = os.path.join(os.path.dirname(__file__), path)
        if os.path.exists(state_dict_path):
            state_dict = torch.load(state_dict_path, device)
            self.load_state_dict(state_dict)
        else:
            raise ValueError('No pretrained model founded.')
...
def SeNet(pretrained):
    net = build_resnet("se-resnext50-32x4d", "classic", 2, False)
    device = 'cuda' if torch.cuda.is_available() else 'cpu'
    net.to(device)
    net.load_weights(pretrained, device)
    return net
